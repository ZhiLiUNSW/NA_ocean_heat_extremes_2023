...
%% ########################################################################
%% 1.0.1 Comparing the total SWR term from Ryan and Maurice
%  Bar Charts and Maps ####################################################
% #########################################################################
% #########################################################################
% V1
...
subplot('position',pos{3})
    h0=bar(5:8,bar_dmlt(5:8,:));
       ...
       % ##################################################################
       % Add SWR from Ryan Holmes
        hold on
        bar_dmlt_Clim(1:12,1:8)=NaN;

        % SWR decomposition
        cd('/Users/z5195509/Documents/6_NA_MHW_MLD/2_1_MLT_Decomposition/')
        load('plot_2023_Heat_Budget_ACCESS_OM2_025_V6_V4_Decomposition_1.mat',...
             'dMLT_Qswr_2023_ano')
        bar_dmlt_Clim(:,3)=dMLT_Qswr_2023_ano(1:12,1); % Qnet - SWR
    ...
    hold on
    legend([h0(1) h0(2) h0(3) h0_Clim(3) h0(4) h0(5) h0(6) h0(7) h0(8)],...
           'MLT tendency','Surface flux term','Shortwave (Maurice)','Shortwave (Ryan)','Latent','Longwave','Sensible','Vertical mixing + entrainment','Advection + other minor terms',...
...
%% 1.0.2 Comparing the Surface or MLD SWR term from Ryan and Maurice
%  Bar Charts and Maps ####################################################
% #########################################################################
% #########################################################################
% V1
...
% #########################################################################
   ...
   disp(['Save to the left screen...'])
      ...
      Qnet_clim=Qnet_clim+Qswr_mld_clim;
%       Qshortwave_clim=Qshortwave_clim;%+Qswr_mld_clim;
      Qshortwave_clim=Qswr_mld_clim;%;+Qshortwave_clim
      clear Qswr_mld_clim
      % ###################################################################
      % ###################################################################
      ...
      Qnet=Qnet+Qswr_mld;
%       Qshortwave=Qshortwave;%+Qswr_mld;
      Qshortwave=Qswr_mld;%+Qshortwave;
      clear Qswr_mld
      % ###################################################################
      % ###################################################################
        ...
        bar_dmlt_Clim(1:12,1:8)=NaN;
            ...
            % MLT budget for 1981-2010
                ...
                disp(['   SWR from IAPv3, Year#1981-2010'])
                 ...
                 % Remove the shortwave penatration
%                  SW_net=SW_surf;%+SW_base; clear SW_surf SW_base
                 SW_net=SW_base; clear SW_surf SW_base
...
%                  SW_net=SW_surf;%+SW_base; clear SW_surf SW_base
    ...
    hold on
%     legend([h0(1) h0(2) h0(3) h0_Clim(3) h0(4) h0(5) h0(6) h0(7) h0(8)],...
%            'MLT tendency','Surface flux term','Surface SWR (Maurice)','Surface SWR (Ryan)','Latent','Longwave','Sensible','Vertical mixing + entrainment','Advection + other minor terms',...
%            'Location','northeast','Orientation','vertical','NumColumns',2)
    legend([h0(1) h0(2) h0(3) h0_Clim(3) h0(4) h0(5) h0(6) h0(7) h0(8)],...
           'MLT tendency','Surface flux term','MLD SWR (Maurice)','MLD SWR (Ryan)','Latent','Longwave','Sensible','Vertical mixing + entrainment','Advection + other minor terms',...
           'Location','northeast','Orientation','vertical','NumColumns',2)
    hold on
    set(legend,'fontsize',17)
    hold on
    ...
    % Legend will show names for each color
    legend() 
    set(gca,'Ylim',[-0.4 1.75],'ycolor','k') 
    set(gca,'YTick',-0.4:0.4:1.6)
    set(gca,'YTickLabel',{'-0.4','0','0.4','0.8','1.2','1.6'},'fontsize',20)
    set(gca,'Xlim',[4.5 8.5]) 
    set(gca,'XTick',4.5:1:8.5)
...
% #########################################################################
% #########################################################################
%% 2.1 Comparing the SWR term in 2023 relative to 1981-2010 climatology - ACCESS OM2
%  Bar Charts and Maps ####################################################
clc; clear
time_ann=(1981:2010)';
...
% MLT budget for 1981-2010
    ...
    disp(['   SWR from IAPv3, Year#1981-2010'])
     ...
     clear SW_net SW_surf SW_base count_yr year
     SW_net_clim =nanmean(SW_net0,4); clear SW_net0
     SW_surf_clim=nanmean(SW_surf0,4); clear SW_surf0
     SW_base_clim=nanmean(SW_base0,4); clear SW_base0   
% #########################################################################
...
SW_net_ano  = SW_net  - SW_net_clim; 
SW_surf_ano = SW_surf - SW_surf_clim; 
SW_base_ano = SW_base - SW_base_clim; 
...
subplot('position',pos{11})
        m_proj('miller','lon',[260.5 379.5],'lat',[0.5 69.5]);  
        m_pcolor(lon_025,lat_025,SW_net_ano(:,:,6)');
        shading flat
        ...
        colormap(gca,color0)
        caxis([-60 60]);
        title('a. Total SWR anom. (June 2023)','fontsize',20,'FontWeight','bold')
        
        
         
subplot('position',pos{12})
        m_proj('miller','lon',[260.5 379.5],'lat',[0.5 69.5]);  
        m_pcolor(lon_025,lat_025,SW_surf_ano(:,:,6)');
        shading flat
        hold on
        ...
        colormap(gca,color0)
        caxis([-60 60]);
        title('b. Surface SWR anom.','fontsize',20,'FontWeight','bold')

        

subplot('position',pos{13})
        m_proj('miller','lon',[260.5 379.5],'lat',[0.5 69.5]);  
        m_pcolor(lon_025,lat_025,SW_base_ano(:,:,6)');
        shading flat
        ...
        colormap(gca,color0)
        caxis([-60 60]);
        title('c. SWR anom. at varing MLB','fontsize',19.5,'FontWeight','bold')
...
subplot('position',pos{21})
        m_proj('miller','lon',[260.5 379.5],'lat',[0.5 69.5]);  
        m_pcolor(lon_025,lat_025,SW_net_ano(:,:,7)');
        shading flat
        ...
        colormap(gca,color0)
        caxis([-60 60]);
        title('d. Total SWR anom. (July 2023)','fontsize',20,'FontWeight','bold')
        
        
         
subplot('position',pos{22})
        m_proj('miller','lon',[260.5 379.5],'lat',[0.5 69.5]);  
        m_pcolor(lon_025,lat_025,SW_surf_ano(:,:,7)');
        shading flat
        hold on
        ...
        colormap(gca,color0)
        caxis([-60 60]);
        title('e. Surface SWR anom.','fontsize',20,'FontWeight','bold')

        

subplot('position',pos{23})
        m_proj('miller','lon',[260.5 379.5],'lat',[0.5 69.5]);  
        m_pcolor(lon_025,lat_025,SW_base_ano(:,:,7)');
        shading flat
        ...
        colormap(gca,color0)
        caxis([-60 60]);
        title('f. SWR anom. at varing MLB','fontsize',19.5,'FontWeight','bold')

        
        hBar1 = colorbar('EastOutside','vertical');
        get(hBar1, 'Position');
        set(hBar1, 'Position', [ixs+3*ixw+2*ixd+0.016 iys+1.5*iyw+1*iyd 0.012 1.0*iyw+1*iyd]);
        set(hBar1, 'ytick',-60:10:60,'yticklabel',{'<-60',[],'-40',[],'-20',[],'0',[],'20',[],'40',[],'>60'},'fontsize',20,'FontName','Arial','LineWidth',1.2,'TickLength',0.060);
        m_text(382,95, '[ W m^{-2} ]','fontsize',18,'FontWeight','normal')
% #########################################################################
% #########################################################################
...
%% 2.2 Comparing the SWR term in 2023 relative to 1981-2010 climatology - IAP and ERA5
%  Bar Charts and Maps ####################################################
clc; clear
time_ann=(1981:2010)';
...
% #########################################################################
    ...
    clear year count
    % fixed MLD
    % mld_monthly=nanmean(mld_monthly(:,:,:,1:30),4);
...
%         z=mld_monthly;
        ...
        SW_base(:,:,:,count_yr)=shortwave_mld;
        
        % figure;imagesc(shortwave_mld(240:360,91:150,1))
        shortwave = shortwave + shortwave_mld;
        SW_net(:,:,:,count_yr)=shortwave;
        
        clear shortwave_mld shortwave
      % ##################################################################   

      count_yr=count_yr+1;
end
clear *_dail_clim count_yr year


SW_net_ano  = SW_net(:,:,:,end)  - nanmean(SW_net(:,:,:,1:30),4); 
SW_surf_ano = SW_surf(:,:,:,end) - nanmean(SW_surf(:,:,:,1:30),4); 
SW_base_ano = SW_base(:,:,:,end) - nanmean(SW_base(:,:,:,1:30),4); 
...
subplot('position',pos{11})
        m_proj('miller','lon',[260.5 379.5],'lat',[0.5 69.5]);  
        m_pcolor(lon_IAP,lat_IAP,SW_net_ano(:,:,6)');
        shading flat
...
subplot('position',pos{12})
        m_proj('miller','lon',[260.5 379.5],'lat',[0.5 69.5]);  
        m_pcolor(lon_IAP,lat_IAP,SW_surf_ano(:,:,6)');
        shading flat
        hold on
...
subplot('position',pos{13})
        m_proj('miller','lon',[260.5 379.5],'lat',[0.5 69.5]);  
        m_pcolor(lon_IAP,lat_IAP,SW_base_ano(:,:,6)');
        shading flat
...
subplot('position',pos{21})
        m_proj('miller','lon',[260.5 379.5],'lat',[0.5 69.5]);  
        m_pcolor(lon_IAP,lat_IAP,SW_net_ano(:,:,7)');
        shading flat
...
subplot('position',pos{22})
        m_proj('miller','lon',[260.5 379.5],'lat',[0.5 69.5]);  
        m_pcolor(lon_IAP,lat_IAP,SW_surf_ano(:,:,7)');
        shading flat
        hold on
...
subplot('position',pos{23})
        m_proj('miller','lon',[260.5 379.5],'lat',[0.5 69.5]);  
        m_pcolor(lon_IAP,lat_IAP,SW_base_ano(:,:,7)');
        shading flat
